#ifndef __ts_Covariance__
#define __ts_Covariance__
...
/**
 * \class Covariance
 * \brief Covariance matrix calculator
 *
 * Populates event samples from selected event trees, iterating through
 * event weights (multisims) to compute a covariance correlation matrices.
 */
class Covariance {
public:
  Covariance();
  ...
  /**
   * Add an input file for processing
   *
   * \param _f The path to the file
   */
  ...
  /**
   * Add a weight to the calculation.
   *
   * \param w The string identifier for the weight in the MCEventWeight map
   */
